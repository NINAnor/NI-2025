...
```{r, warning = FALSE}
rm(simData, indicatorData, indicatorImport)
```
...
The goal for the Nature index database is that those visualizations 
For 1., we ultimately also want to include a line for the area-aggregated "indicator index". Calculation thereof is not included in the code yet, but will be added soon.
...
- In this document, we will display 1. featuring all areas and 2. for the most recent year, 2024. Maps for previous years have not been assembled yet, but these will be part of the presentation of data and results on the new NI webpage.
...
## Set time-series plot color for indicators with > 20 areas
plot.col <- "#D37295FF"
...
Then, we proceed to plot time series and maps: 
```{r}
for(i in 1:nrow(indicatorList)){
  ...
  data_rows <- c("mean", "median", "q025", "q05", "q25", "q75", "q90", "q975") 
