...
for(i in 1:nrow(indicatorList)){
  ...
  data_indIdx <- readRDS(paste0("results_Indicators/indIndex_id_", ind_id, ".rds"))$summary %>%
    dplyr::filter(indexArea == "wholeArea" & year_t %in% ind_focalYears) %>%
...
Below, we will show the time series plots for raw and scaled indicator data. 
The render has not been implemented properly yet and for the time being, we refer to the individual plot files saved under "plots". 
...
for(i in 1:nrow(indicatorList)){
  ...
  #cat('Raw (unscaled) indicator data: \n')
  #cat('\n')
  #print(TS_raw[[i]])
  
  #cat('\n\n')
  
  #cat('Scaled indicator data including weighted averages: \n')
  #cat('\n')
  #print(TS_scaled[[i]])
