...
### A note on reference value uncertainty {.unnumbered}
...
shapeLibraryPath <- Sys.getenv("NI_shapeLibraryPath", unset = "Shapefiles")
...
NIcalc::plotWeights(Index$wholeArea$'2024', main = paste0("NI2025 (", Ecosystem[2], ")"), xlab = "Vekt")
dev.off()

pdf(paste0(plotPath, "/weightsAll.pdf"), width = 6, height = 9)
NIcalc::plotWeights(Index$wholeArea$'2024', allBars = TRUE, main = paste0("NI2025 (", Ecosystem[2], ")"), xlab = "Vekt")
dev.off()
...
```{r, include = FALSE}
standardTS1_n <- NIflex::plotNI_StandardTS(Index = Index,
                                           plotTitle = Ecosystem[2],
                                           addAverage = TRUE,
                                           truncateY = FALSE,
                                           norwegian = TRUE)
standardTS2_n <- NIflex::plotNI_StandardTS(Index = Index,
                                           plotTitle = Ecosystem[2],
                                           addAverage = TRUE,
                                           truncateY = TRUE,
                                           norwegian = TRUE)
...
ggsave(file = paste0(plotPath, "/TimeSeries_standard_norsk.svg"), plot = standardTS1_n, width = 7, height = 4)

ggsave(file = paste0(plotPath, "/TimeSeries_standard.pdf"), plot = standardTS1, width = 7, height = 4)
ggsave(file = paste0(plotPath, "/TimeSeries_standard_norsk.pdf"), plot = standardTS1_n, width = 7, height = 4)

ggsave(file = paste0(plotPath, "/TimeSeries_standard_trunc.svg"), plot = standardTS2, width = 7, height = 4)
ggsave(file = paste0(plotPath, "/TimeSeries_standard_trunc_norsk.svg"), plot = standardTS2_n, width = 7, height = 4)

ggsave(file = paste0(plotPath, "/TimeSeries_standard_trunc.pdf"), plot = standardTS2, width = 7, height = 4)
ggsave(file = paste0(plotPath, "/TimeSeries_standard_trunc_norsk.pdf"), plot = standardTS2_n, width = 7, height = 4)
```
...
```
```{r, include = FALSE}
densTS1_n <- NIflex::plotNI_DensityRidgeTS(Index = Index, 
                                           OutputType = OutputType, 
                                           ecosystem = ecosystem_use, 
                                           theme = theme,
                                           allAreas = FALSE, 
                                           selectedArea  = "wholeArea",
                                           norwegian = TRUE)
densTS2_n <- NIflex::plotNI_DensityRidgeTS(Index = Index, 
                                           OutputType = OutputType, 
                                           ecosystem = ecosystem_use, 
                                           theme = theme,
                                           allAreas = TRUE,
...
ggsave(file = paste0(plotPath, "/TimeSeries_density_norsk.svg"), plot = densTS1_n, width = 7, height = 5)

ggsave(file = paste0(plotPath, "/TimeSeries_density.pdf"), plot = densTS1, width = 7, height = 5)
ggsave(file = paste0(plotPath, "/TimeSeries_density_norsk.pdf"), plot = densTS1_n, width = 7, height = 5)

ggsave(file = paste0(plotPath, "/TimeSeries_density_byArea.svg"), plot = densTS2, width = 7, height = 5)
ggsave(file = paste0(plotPath, "/TimeSeries_density_byArea_norsk.svg"), plot = densTS2_n, width = 7, height = 5)

ggsave(file = paste0(plotPath, "/TimeSeries_density_byArea.pdf"), plot = densTS2, width = 7, height = 5)
ggsave(file = paste0(plotPath, "/TimeSeries_density_byArea_norsk.pdf"), plot = densTS2_n, width = 7, height = 5)
```

### Median, uncertainty, and displacement maps
...
## Addendum: thematic indices

As of NI2025, all previously published thematic indices have been discontinued. 
We have, however, had a request to provide the thematic indices for "Forsuring ferskvann" (= freshwater acidification) and "Eutrofiering ferskvann" (= freshwater eutrophication) as tools to aid with interpretation of NI for freshwater ecosystems. 
...
## Turn off key indicator and trophic group weighing
KeyIndicators <- FALSE
TrophicWeights <- FALSE
...
```{r, echo = FALSE, eval = FALSE}
## Set ecosystem to null (thematic indices)
ecosystem_use <- NULL
...
## Calculate index for specified ecosystem/indicators
theme_acidification <- calculateCustomNI(ecosystem = ecosystem_use,
                              indicators = indicators_use,
                              theme = theme,
                              dropInd = dropInd,
                              username = NIdb_username,
...
## Save output
file.rename(from = "CustomIndex_Data.rds",
            to = "NI2025_Data_Freshwater_themeAcidification.rds")
```

```{r, echo = FALSE}
theme_acidification <- readRDS("NI2025_Data_Freshwater_themeAcidification.rds")
```

The following table shows the indicators that are part of the thematic index for freshwater acidification: 

```{r, echo = FALSE}
ind_table <- theme_acidification$importData$indicators %>%
  dplyr::select(id, name, functionalGroup, scalingModel)

knitr::kable(ind_table,
             caption = "List of indicators included in thematic index for freshwater acidification. id and name are consistent with use in the NI database. Key indicator (nøkkelindikator) weights are not used for calculating thematic indices. Functional group also affects the weight attributed to indicators. Scaling model = Low means that indicator values > reference are considered good, while scaling model Max assumes that the reference is a maximum threshold, and values exceeding it are bad.")
```
...
```{r, echo = FALSE}
NIcalc::plotWeights(theme_acidification$CustomIndex$wholeArea$`2024`,
                    main = "Freshwater acidification",
                    keycol = "grey", barcol = "grey")
```
...
NIflex::plotNI_StandardTS(Index = theme_acidification$CustomIndex,
                          plotTitle = "Freshwater acidification",
                          addAverage = TRUE,
                          truncateY = TRUE)
```
...
```{r, warning = FALSE, message = FALSE, echo = FALSE}
NIflex::plotNI_DensityRidgeTS(Index = theme_acidification$CustomIndex, 
                                      OutputType = OutputType, 
                                      ecosystem = "Ferskvann", 
                                      theme = theme,
                                      allAreas = TRUE)
...
## Calculate index for specified ecosystem/indicators
theme_eutrophication <- calculateCustomNI(ecosystem = ecosystem_use,
                                          indicators = indicators_use,
                                          theme = theme,
                                          dropInd = dropInd,
                                          username = NIdb_username,
...
## Save output
file.rename(from = "CustomIndex_Data.rds",
            to = "NI2025_Data_Freshwater_themeEutrophication.rds")
```

```{r, echo = FALSE}
theme_eutrophication <- readRDS("NI2025_Data_Freshwater_themeEutrophication.rds")
```

The following table shows the indicators that are part of the thematic index for freshwater eutrophication: 

```{r, echo = FALSE}
ind_table <- theme_eutrophication$importData$indicators %>%
  dplyr::select(id, name, functionalGroup, scalingModel)

knitr::kable(ind_table,
             caption = "List of indicators included in thematic index for freshwater eutrophication. id and name are consistent with use in the NI database. Key indicator (nøkkelindikator) weights are not used for calculating thematic indices. Functional group also affects the weight attributed to indicators. Scaling model = Low means that indicator values > reference are considered good, while scaling model Max assumes that the reference is a maximum threshold, and values exceeding it are bad.")
```
...
```{r, echo = FALSE}
NIcalc::plotWeights(theme_eutrophication$CustomIndex$wholeArea$`2024`,
                    main = "Freshwater eutrophication",
                    keycol = "grey", barcol = "grey")
```
...
```{r, warning = FALSE, message = FALSE, echo = FALSE}
NIflex::plotNI_StandardTS(Index = theme_eutrophication$CustomIndex,
                          plotTitle = "Freshwater eutrophication",
                          addAverage = TRUE,
                          truncateY = TRUE)
```
...
```{r, warning = FALSE, message = FALSE, echo = FALSE}
NIflex::plotNI_DensityRidgeTS(Index = theme_eutrophication$CustomIndex, 
                                      OutputType = OutputType, 
                                      ecosystem = "Ferskvann", 
                                      theme = theme,
                                      allAreas = TRUE)
...
for(t in 1:length(years)){
  ...
  ## Draw maps
  mapMedian_n <- NIflex::plotNI_Map(shp = CustomNI_map, 
                                  year = years[t], 
                                  OutputType = OutputType, 
                                  ecosystem = ecosystem_use,
                                  plotMedian = TRUE, plotCI = FALSE, plotDisplacement = FALSE,
                                  norwegian = TRUE,
                                  interactiveMap = FALSE)
  mapCI_n <- NIflex::plotNI_Map(shp = CustomNI_map, 
                              year = years[t], 
                              OutputType = OutputType, 
                              ecosystem = ecosystem_use,
                              plotMedian = FALSE, plotCI = TRUE, plotDisplacement = FALSE, 
                              norwegian = TRUE,
                              interactiveMap = FALSE)
  mapDisp_n <- NIflex::plotNI_Map(shp = CustomNI_map, 
                                year = years[t], 
                                OutputType = OutputType, 
                                ecosystem = ecosystem_use,
                                plotMedian = FALSE, plotCI = FALSE, plotDisplacement = TRUE, 
  ...
  ## Save as multipage pdf
  pdf(paste0(plotPath, "/Maps/NI2025_", Ecosystem[1], "_", years[t], "_norsk.pdf"), width = 5, height = 5)
  ...
  ## Save as svg
  svglite(paste0(plotPath, "/Maps/NI2025_", Ecosystem[1], "_Median_", years[t], "_norsk.svg"), width = 5, height = 5)
  print(mapMedian_n)
  dev.off()
  
  svglite(paste0(plotPath, "/Maps/NI2025_", Ecosystem[1], "_CIwidth_", years[t], "_norsk.svg"), width = 5, height = 5)
  print(mapCI_n)
  dev.off()
  
  svglite(paste0(plotPath, "/Maps/NI2025_", Ecosystem[1], "_Displacement_", years[t], "_norsk.svg"), width = 5, height = 5)
  print(mapDisp_n)
  dev.off()
}
```