...
```{r}
knitr::kable(theme_acidification$importData$indicators,
             caption = "List of indicators included in thematic index for freshwater acidification. id and name are consistent with use in the NI database. Indicators with keyElement = TRUE are treated as key indicators (nøkkelindikatorer) and assigned extra weight. Functional group also affects the weight attributed to indicators. Scaling model = Low means that indicator values > reference are considered good, while scaling model Max assumes that the reference is a maximum threshold, and values exceeding it are bad.")
```

The indicator weights are shown below. 

```{r, echo = FALSE}
NIcalc::plotWeights(theme_acidifcation$CustomIndex$wholeArea$`2024`,
...
```{r, warning = FALSE, message = FALSE}
NIflex::plotNI_StandardTS(Index = theme_acidifcation$CustomIndex,
...
```{r, warning = FALSE, message = FALSE}
NIflex::plotNI_DensityRidgeTS(Index = Index, 
                                         OutputType = OutputType, 
                                         ecosystem = "Ferskvann", 
                                         theme = theme,
                                         allAreas = TRUE)
...
```{r}
knitr::kable(theme_eutrophication$importData$indicators,
             caption = "List of indicators included in thematic index for freshwater eutrophication. id and name are consistent with use in the NI database. Indicators with keyElement = TRUE are treated as key indicators (nøkkelindikatorer) and assigned extra weight. Functional group also affects the weight attributed to indicators. Scaling model = Low means that indicator values > reference are considered good, while scaling model Max assumes that the reference is a maximum threshold, and values exceeding it are bad.")
```
